...
describe('send recovery service', () => {
  it('queues force resend only for uncertain recipients with acknowledgement', async () => {
    const repository = {
      listSendOutcomes: vi.fn().mockResolvedValue([{ campaignId: 'campaign-1', contactId: 'contact-1', contactEmail: 'a@example.com', contactName: 'A', sendStatus: 'uncertain', requiresReview: true, forceResendAllowed: true }]),
  ...
  it('rejects submitted recipients', async () => {
    const service = new SendRecoveryService({
      listSendOutcomes: vi.fn().mockResolvedValue([{ contactId: 'contact-1', sendStatus: 'submitted', forceResendAllowed: false }])
    } as never);
