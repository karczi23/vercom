...
export function CampaignEditorToolbar({ onBold, onItalic, onHeading, onFont }: CampaignEditorToolbarProps) {
  return (
    <div className="flex flex-wrap items-center gap-2 border-b border-slate-200 bg-slate-50 p-2" aria-label="Campaign editor toolbar">
      ...
      <select className="h-9 rounded-md border border-slate-300 bg-white px-2 text-sm text-slate-900" aria-label="Heading level" onChange={event => onHeading(Number(event.target.value) as 1 | 2 | 3 | 4 | 5 | 6)} defaultValue="">
        <option value="" disabled>H</option>
        {headingLevels.map(level => <option key={level} value={level}>H{level}</option>)}
      </select>
